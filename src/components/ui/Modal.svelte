...
>
  <div
    transition:scale
    class="flex flex-col mx-auto w-11/12 max-w-2xl rounded-md border border-accent sm:w-5/6 lg:w-1/2"
  >
    <div class="flex flex-row justify-between p-6 text-primary bg-darkest rounded-t-md">
      <p class="font-bold">{title}</p>
      <CloseCross on:click={close} />
    </div>
    <div class="p-5 bg-darker"><slot /></div>
    <div class="flex flex-row justify-between items-center p-5 bg-darkest rounded-b-md">
